...
import Pareto.metrics
import copy

from Pareto.RewardApproximator import RewardApproximator
from Pareto.NonDominatedApproximator import NonDominatedApproximator
from Pareto.Estimator import Estimator



class Pareto:
    def __init__(self,env,number_of_states = 4, number_of_actions = 4,metrs = None,step_start_learning = 1000,numberofeps = 1000, ReplayMem = None,number_of_p_points = 10,epsilon_start = 1.,epsilon_decay = 0.99997,epsilon_min = 0.01,gamma = 0.98,copy_every=100,ref_point = [-1,-2] ):
    ...
    def estimate_reward(self,state,action):
        self.statesActions = np.zeros((1, self.number_of_states+1))
    ...
    def pareto_frontier_policy_training(self,polDict):
        ...
        memory_capacity = 100
        D = ReplayMemory((self.number_of_states,),size= memory_capacity, nO=2)
        number_of_actions = 4
        ...
        while e < self.numberofeps:
            ...
            one_hot_state = np.zeros(self.number_of_states)
            terminal = False
            ...
            while terminal is False and step < MAX_STEPS:
                ...
                ohe_next_state= np.zeros(self.number_of_states)
